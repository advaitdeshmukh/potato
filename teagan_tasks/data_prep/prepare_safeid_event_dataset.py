...
# Paths resolve against the event-relation task directory regardless of where
# this script is invoked from.
TASKS_DIR = Path(__file__).resolve().parent.parent
EVENT_DATA = TASKS_DIR / "event_relation_annotation_task" / "data"

DEFAULT_INPUT = EVENT_DATA / "dolma_final_sample_s42_n1250_t0.5_llm_summary.csv"
DEFAULT_OUTPUT = EVENT_DATA / "dolma_final_sample_s42_n1250_t0.5_llm_summary_safeid_with_spans.csv"
...
# Columns that Potato reads via pandas: empty cells become float NaN, which
# json.dumps serialises as the literal token NaN — invalid JSON.  Write a
# fallback string so pandas never sees an empty cell for these fields.
_NAN_FALLBACKS: dict[str, str] = {
    ...
    with input_path.open("r", encoding="utf-8", newline="") as in_f:
        ...
        with output_path.open("w", encoding="utf-8", newline="") as out_f:
            ...
            for idx, row in enumerate(reader):
                ...
                for col, fallback in _NAN_FALLBACKS.items():
                    if col in out_row and not out_row[col]:
                        out_row[col] = fallback
